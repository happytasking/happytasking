...
export function parseNextActionPayload(body: string): { rows: AiTrainingRoleRow[]; total: number } {
  const line = body
    ...
    .find((row) => /^1:\{/.test(row));
  if (!line) {
    throw new Error("AITraining.jobs fetchRoles payload missing");
  }
...
async function resolveActionId(base: string, configured: string): Promise<string> {
  try {
    ...
    if (!chunk) return configured;
    const js = await fetchAllowed(new URL(chunk, base).toString(), {
      allowedHosts: AITRAINING_ALLOWED_HOSTS,
    });
    return discoverFetchRolesActionId(js.body) || configured;
  } catch {
    return configured;
  }
}
  ...
  return {
    ...
    async fetch(opts = {}): Promise<AdapterFetchResult> {
      ...
      const actionId = await resolveActionId(
        baseUrl,
        overrides.actionId || env.AITRAINING_JOBS_FETCH_ROLES_ACTION,
      );
      ...
      while (true) {
        ...
        if (parsed.rows.length < AITRAINING_PAGE_SIZE && maxRecords == null) {
          // Observed server cap is 60; keep paging until empty or total reached.

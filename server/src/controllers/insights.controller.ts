...
import { pageviewSchema, recordPageview, publicEventSchema, recordPublicEvent } from "../services/visit.service.js";
...
export const publicEvent = asyncHandler(async (req: Request, res: Response) => {
  const parsed = publicEventSchema.safeParse(req.body);
  if (!parsed.success) {
    throw new ApiError(400, "Validation failed", parsed.error.issues);
  }
  const result = await recordPublicEvent(
    parsed.data,
    requestMeta(req),
    req.user?.id,
  );
